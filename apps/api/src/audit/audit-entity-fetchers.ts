import type { PrismaService } from "../prisma/prisma.service";

export type EntityFetcher = (id: string, prisma: PrismaService) => Promise<unknown>;

/**
 * เพิ่ม entity ใหม่ที่นี่คู่กับทุกครั้งที่ใช้ @AuditEntity("XxxYyy") ใน controller — ถ้าไม่เพิ่ม
 * AuditInterceptor จะบันทึก log ได้แต่ before จะเป็น null เสมอ (ไม่ผิด แค่ diff จะโชว์ไม่ครบ)
 */
export const AUDIT_ENTITY_FETCHERS: Record<string, EntityFetcher> = {
  Branch: (id, prisma) => prisma.client.branch.findUnique({ where: { id } }),
  Staff: (id, prisma) => prisma.client.staffProfile.findUnique({ where: { id } }),
  Room: (id, prisma) => prisma.client.room.findUnique({ where: { id } }),
  RoomType: (id, prisma) => prisma.client.roomType.findUnique({ where: { id } }),
  Service: (id, prisma) =>
    prisma.client.service.findUnique({ where: { id }, include: { variants: true } }),
  ServiceCategory: (id, prisma) => prisma.client.serviceCategory.findUnique({ where: { id } }),
  ServiceVariant: (id, prisma) => prisma.client.serviceVariant.findUnique({ where: { id } }),
  ShiftTemplate: (id, prisma) => prisma.client.shiftTemplate.findUnique({ where: { id } }),
  StaffShift: (id, prisma) => prisma.client.staffShift.findUnique({ where: { id } }),
  StaffLeave: (id, prisma) => prisma.client.staffLeave.findUnique({ where: { id } }),
  Member: (id, prisma) => prisma.client.member.findUnique({ where: { id } }),
  AppointmentItem: (id, prisma) => prisma.client.appointmentItem.findUnique({ where: { id } }),
  StaffQueueEntry: (id, prisma) => prisma.client.staffQueueEntry.findUnique({ where: { id } }),
  Package: (id, prisma) => prisma.client.package.findUnique({ where: { id } }),
  MemberPackage: (id, prisma) => prisma.client.memberPackage.findUnique({ where: { id } }),
  Promotion: (id, prisma) => prisma.client.promotion.findUnique({ where: { id } }),
  Coupon: (id, prisma) => prisma.client.coupon.findUnique({ where: { id } }),
  Bill: (id, prisma) => prisma.client.bill.findUnique({ where: { id }, include: { lines: true, payments: true } }),
  CashierShift: (id, prisma) => prisma.client.cashierShift.findUnique({ where: { id } }),
  StaffProfile: (id, prisma) => prisma.client.staffProfile.findUnique({ where: { id } }),
  TimeClockEntry: (id, prisma) => prisma.client.timeClockEntry.findUnique({ where: { id } }),
  BillTip: (id, prisma) =>
    prisma.client.billTip.findUnique({ where: { id }, include: { allocations: true } }),
  PayrollPeriod: (id, prisma) =>
    prisma.client.payrollPeriod.findUnique({ where: { id }, include: { summaries: true } }),
};
