...
- **`BillTip` แยกขาดจาก `BillPayment` โดยสิ้นเชิง — เงินทิปสดที่รับมาไม่ถูกนับเข้า `CashierShift.systemCashSatang`
  เลย** ตามที่ ADR-031 เตือนไว้ล่วงหน้าแล้วว่าต้องกลับมาตัดสินใจตอน T6.3 — รอบนี้ตัดสินใจแค่ "ยังไม่รวม" (ไม่ใช่
  ตัดสินใจถาวรว่าไม่ควรรวม) เพราะ T5.7 ปิดไปแล้วและไม่มี requirement ชัดเจนให้แก้ ผลคือ: ถ้าลูกค้าทิปเป็นเงินสด
  ลิ้นชักจะมีเงินสดจริงมากกว่ายอดระบบเสมอเท่ากับยอดทิปเงินสดของวันนั้น ต้องอธิบายเป็น `varianceReason` ทุกรอบ
  ปิดกะที่มีทิปเงินสด — **ต้องกลับมาตัดสินใจจริงถ้าร้านเริ่มรับทิปเงินสดบ่อย** ว่าจะรวมเข้า `systemCashSatang`
  หรือแยกช่องนับต่างหากในหน้าปิดรอบกะ
...
ผลกระทบ/ข้อควรระวังสำหรับอนาคต: `docs/DOMAIN.md` ข้อ 14/16 ยังเขียนบรรยายพฤติกรรมเดิม (บังคับ PIN ทุกกรณี)
ยังไม่ได้อัปเดตให้ตรงกับพฤติกรรมใหม่นี้ — ควรปรับข้อความให้สะท้อนเงื่อนไขผ่อนนี้ไว้ด้วยในรอบถัดไปที่แก้เอกสารนั้น
