...
## ADR-048: ซ่อนเมนู "คลัง" และ "ตั้งค่า" ออกจากกระดานเมนูซ้ายชั่วคราว — หน้ายังไม่ถูกสร้างจริง (404)
วันที่: 2026-08-28
Task ที่เกี่ยวข้อง: ตามหลัง ADR-047 (ไม่มีใน docs/PLAN.md — เป็นการแก้ไขจากฟีดแบ็กเจ้าของร้านหลังทดลองใช้)

บริบท: เจ้าของร้านลองกดเมนู "คลัง" และ "ตั้งค่า" ที่กระดานเมนูซ้าย (หลัง ADR-047) แล้วเจอ 404 —
ตรวจสอบพบว่า `nav-items.ts` เดิม (ก่อน ADR-047 ก็มีอยู่แล้ว) มี `href: "/inventory"` และ `href: "/settings"`
ทั้งที่ `apps/web/src/app/(app)/inventory` และ `.../settings` **ไม่เคยมีโฟลเดอร์หน้าเว็บจริงเลย** — ไม่ใช่บั๊กที่
ADR-047 สร้างขึ้นใหม่ แค่ ADR-047 ทำให้เมนูเด่นขึ้น (มีไอคอน+จัดกลุ่ม) เลยมีคนกดเจอ ตรวจสอบ docs/PLAN.md §12
ยืนยันว่าทั้งสองเรื่องเป็นส่วนเสริมที่ตั้งใจเลื่อนไปทำทีหลัง: T12.3 (คลัง: สินค้า/วัตถุดิบ+BOM+ตัดสต๊อกอัตโนมัติ)
และ T12.7 (ตั้งค่าระบบละเอียด: กติกาคิว/เพดานส่วนลด/รูปแบบใบเสร็จ/ภาษี) — ยังไม่ถึงคิวสร้างจริง

ตัดสินใจ: ลบ 2 รายการนี้ออกจาก `NAV_ITEMS` (`nav-items.tsx`) ชั่วคราว ไม่ลบ `InventoryIcon`/`SettingsIcon`
ออกจาก `nav-icons.tsx` (เผื่อใช้ตอนสร้างหน้าจริงใน T12.3/T12.7) — ไม่สร้างหน้า placeholder เปล่าๆ แทน
เพราะจะทำให้ดูเหมือนฟีเจอร์ทำงานได้ทั้งที่ยังไม่มีอะไรข้างหลัง (สร้างความสับสนมากกว่า 404 ตรงๆ)

ผลกระทบ: ผู้ใช้ทุกบทบาทมองไม่เห็นเมนู 2 รายการนี้จนกว่าจะมีคนสร้าง T12.3/T12.7 จริง (ตอนนั้นต้องเพิ่ม entry
กลับเข้า `NAV_ITEMS` ด้วย ไม่ใช่แค่สร้างหน้าเว็บเฉยๆ — จุดที่ต้องจำ)

---

## ADR-049: เพิ่มปุ่ม "เข้าคิว" ที่กระดานคิว — เดิม T4.4 มี API ครบแต่ไม่มี UI ให้กดเลย
วันที่: 2026-08-28
Task ที่เกี่ยวข้อง: T4.4 (ตามหลัง ADR-047/048 — ไม่มีใน docs/PLAN.md เดิม เป็นช่องโหว่ที่พบระหว่างอธิบาย
กติกาคิวหมุนให้เจ้าของร้านฟัง)

บริบท: `POST /branches/:branchId/staff-queue/join` และ pure function `joinQueue` (staff-queue.ts) สร้างไว้
ครบตั้งแต่ T4.4 พร้อม unit test แต่ตรวจสอบพบว่า **ไม่มีหน้าเว็บไหนเรียก endpoint นี้เลยสักที่** — `AppointmentItemController`
มี fallback auto-create แถวคิวให้ก็จริง แต่เกิดขึ้น**เฉพาะตอนนัด "แรก" ของพนักงานคนนั้นในวันนั้นจบ/ยกเลิก/
ไม่มา** เท่านั้น (ดู `assignQueueAfterStatusChange` ใน `appointment-item.controller.ts`) ผลคือพนักงานที่ยังไม่มี
นัดเลยในวันนั้น (เช่นตอนเช้าก่อนเปิดร้าน) **ไม่มีทางเข้าคิวได้เองเลย** ต้องรอให้มีคนอื่นได้งานไปก่อนคิวถึงจะ
เริ่มมีลำดับ — เป็นช่องโหว่จริงที่กระทบการใช้งานทุกเช้า ไม่ใช่แค่ความสวยงาม

ตัดสินใจ: เพิ่มส่วน "ยังไม่เข้าคิว" ใน `QueueRail` (`apps/web/.../board/queue-rail.tsx`) — แสดงรายชื่อ
พนักงานที่ยัง active แต่ยังไม่มีแถวคิวของวันที่กำลังดูอยู่ (คำนวณจาก `staffApi.list` ตัดกับ `staffQueueApi.list`
ที่ `board-page-client.tsx` โหลดอยู่แล้วทั้งคู่) พร้อมปุ่ม "+" ต่อคน เรียก `staffQueueApi.join` ที่มีอยู่แล้ว —
**ไม่แก้ backend เลยแม้แต่บรรทัดเดียว** เพราะ API/logic ถูกต้องสมบูรณ์อยู่แล้ว ขาดแค่ทางเข้าฝั่ง UI
ซ่อนส่วนนี้เมื่อไม่มีสิทธิ์ `manage booking` (เงื่อนไขเดียวกับปุ่ม "+ จองด่วน" ที่มีอยู่แล้วในหน้านี้)

ผลกระทบ: ไม่มี — เป็นการเพิ่ม UI ล้วนๆ ต่อยอด endpoint/pure function ที่มี test ครอบอยู่แล้วตั้งแต่ T4.4

---

## ADR-050: เปิดแค่ 6 เมนูแรกบนกระดานเมนูซ้ายสำหรับ "Basic Package" ร้านพนักงานน้อย
วันที่: 2026-08-28
Task ที่เกี่ยวข้อง: ไม่มีใน docs/PLAN.md (เจ้าของร้านขอปรับขอบเขตฟีเจอร์ที่เปิดให้เห็นสำหรับร้านเล็ก)

บริบท: หลังคุยเรื่องฟังก์ชันที่เหมาะกับร้านพนักงาน 4-8 คน เจ้าของร้านตัดสินใจให้เปิดเผยแค่ 6 เมนูแกนหลัก
ก่อน ส่วนที่เหลือ (ลงเวลาเข้า-ออกงาน, ค่ามือ, โปรโมชั่น, ห้อง/เตียง, รายงาน) ยังไม่พร้อมให้ร้านเล็กใช้ตอนนี้ —
เหตุผลหลักของแต่ละอย่าง: ค่ามือมีเรตคอมมิชชั่นเป็นค่าสมมติยังไม่ยืนยันจริง (T6.2), ที่เหลือเป็น "ดีมีประโยชน์
แต่ไม่จำเป็นวันแรก" ตามที่วิเคราะห์ไว้

ตัดสินใจ: ตัด 5 รายการนี้ออกจาก `NAV_ITEMS` (`apps/web/.../nav-items.tsx`) เหลือ 6 เมนู: กระดานคิว, บิล/
แคชเชียร์, สมาชิก, บริการ, คอร์ส/แพ็กเกจ, พนักงาน (+ แดชบอร์ดที่แสดงเสมออยู่แล้ว) — **ไม่แตะสิทธิ์ RBAC/
backend เลย** หน้าที่ถูกซ่อนยังทำงานได้ปกติทุกอย่างถ้าพิมพ์ URL ตรง ๆ (ต่างจาก ADR-048 ที่ซ่อนเพราะหน้ายังไม่มี
จริง — รอบนี้หน้ามีจริงแต่ยังไม่อยากให้ร้านเล็กเห็น) เป็นการซ่อนระดับ "การจัดวางเมนู" (presentation) ล้วน ๆ
ไม่ใช่การปิดฟีเจอร์จริง

ไม่ได้สร้างระบบสวิตช์เปิด/ปิดโมดูลต่อสาขา (มีเสนอไว้แต่เจ้าของร้านยังไม่สั่งให้ทำ) — ถ้าต้องการสลับกลับ
ตอนนี้ต้องแก้ไฟล์ `nav-items.tsx` ตรง ๆ เอา entry ที่ตัดออกกลับเข้าไป (มีคอมเมนต์ในไฟล์บอกไว้แล้ว) ระบบ
สวิตช์จริงต่อสาขาควรทำพร้อมกับ T12.7 (หน้าตั้งค่า) เมื่อถึงคิว

ผลกระทบ: บทบาทไหนก็ตามที่เคยเห็น 5 เมนูนี้จะไม่เห็นในกระดานเมนูซ้ายอีกต่อไป จนกว่าจะมีคนแก้ไฟล์กลับ —
ข้อมูล/ฟีเจอร์เบื้องหลังไม่ได้หายไปไหน (เช่น ถ้าเคยมีนัดที่ผูก promotion ไว้ก็ยังทำงานถูกต้องเหมือนเดิม)

---

## ADR-051: เขียนสเปก Mobile/Tablet UX ใหม่ทั้งระบบ — ยังไม่แก้โค้ด
วันที่: 2026-09-18
Task ที่เกี่ยวข้อง: ไม่มีใน docs/PLAN.md เดิม (ผู้ใช้ขอผ่าน `/manage-programming-project` ให้ "design หน้าจอ
สำหรับ Mobile ใหม่ เน้นใช้งานง่ายเหมือน mobile app ให้ Support Tablet และ Mobile")

บริบท: หน้าจอทั้ง 11 หน้าที่มีอยู่ (กระดานคิว, พนักงาน, บริการ, ห้อง/เตียง, สมาชิก, คอร์ส/แพ็กเกจ, โปรโมชั่น,
บิล/แคชเชียร์, รายงาน, ค่ามือ, ลงเวลา) ออกแบบมาเพื่อจอกว้าง ≥ 1024px ทั้งหมด (`AppShell` sidebar 240px,
`Table` แบบคอลัมน์กว้าง, `Sheet` เลื่อนจากขวากว้างคงที่, Lane Board เป็น grid เวลา×คน) ผู้ใช้ระบุขอบเขตชัดว่า
ต้องการ "ออกแบบ mobile UX ใหม่ทั้งระบบ" (เปลี่ยน navigation pattern เป็น bottom nav/card-list ไม่ใช่แค่ทำให้
ของเดิม responsive) และให้ทำ **design spec ก่อน** ไม่ลงมือแก้โค้ดในเซสชันนี้ — ตรงตามกติกา CLAUDE.md
"ทำเฉพาะ Task ที่ได้รับมอบหมาย" และ "เมื่อไม่แน่ใจ หยุดแล้วถาม" เพราะงานนี้ไม่ตรงกับ Task ใดใน PLAN.md
โดยตรงและกระทบหลายหน้าพร้อมกัน

ตัดสินใจ: เขียนสเปกไว้ที่ `docs/DESIGN-MOBILE.md` (ไฟล์แยกชั่วคราว ไม่แก้ `docs/DESIGN.md` เดิมเพื่อไม่ให้
กระทบเอกสารอ้างอิงหลักก่อนมีการยืนยัน) ครอบคลุม: breakpoint 3 ระดับ (mobile < 768 / tablet 768–1023 /
desktop ≥ 1024 — ใช้ค่า default ของ Tailwind v4 ที่ระบบมีอยู่แล้ว ไม่ override), bottom tab bar แทน
hamburger+drawer เดิมบนจอมือถือ (4 เมนูหลักจาก 6 เมนู Basic Package ของ ADR-050 + ปุ่ม "เพิ่มเติม"),
component ใหม่ 3 ตัว (`ResponsiveList` แทน `Table`, full-screen bottom `Sheet`, FAB), และ pattern เฉพาะ
หน้าที่ซับซ้อน (Lane Board → มุมมองรายคนแนวตั้งบนมือถือ ไม่ทำ drag-and-drop บนจอสัมผัสเล็ก, บิล/แคชเชียร์ →
wizard 3 ขั้นแทนหลายคอลัมน์พร้อมกัน) เสนอ milestone ใหม่ **M10** ใน PLAN.md (T10.1–T10.9) ให้ผู้ใช้ยืนยัน
ก่อนเพิ่มเข้า PLAN.md จริงและเริ่มเขียนโค้ด

**ไม่แตะโค้ดใดๆ ในเซสชันนี้** — ไม่มีการแก้ `packages/ui`, `apps/web`, หรือไฟล์ config ใดๆ ทั้งสิ้น
สีชุด/ฟอนต์/สีสถานะ/motion ใน DESIGN.md เดิมไม่เปลี่ยนแปลง — สเปกใหม่อ้างอิงใช้ทั้งหมด ไม่เพิ่มโทเค็นใหม่
...
## ADR-057: เปลี่ยนสีหลักจากปิโตรเลียมทีลเป็นทอง/บรอนซ์ + เขียวใบไม้ ให้เข้ากับโลโก้ Sabaizy
วันที่: 2026-09-20
Task ที่เกี่ยวข้อง: เพิ่มโลโก้ Sabaizy เข้าแอป + ปรับโทนสีให้เข้ากับโลโก้ (คำสั่งจากผู้ใช้ผ่าน /ui-ux-pro-max)

บริบท: หลังเปลี่ยนชื่อระบบเป็น "Sabaizy" ผู้ใช้ส่งโลโก้มาให้ (มือถือดอกลีลาวดีสีทอง/บรอนซ์ + ใบไม้เขียว บน
พื้นครีม) แล้วขอให้เอาโลโก้ไปใส่ในแอปพร้อมปรับโทนสีให้เข้ากัน สีหลักเดิม (`--celadon`) เป็นปิโตรเลียมทีล
(ADR-047) ซึ่งเป็นโทนเย็นตรงข้ามกับโทนอุ่นของโลโก้โดยสิ้นเชิง

ตัดสินใจ: เปลี่ยนค่า hex ของ `--celadon*` (สีหลัก) เป็นทอง/บรอนซ์ตามโลโก้ และย้ายค่าเดิมของ `--brass*`
(ซึ่งเดิมเป็นสีทองใกล้เคียงกับโลโก้อยู่แล้ว) ไปเป็น `--celadon*` แทน แล้วกำหนด `--brass*` ใหม่เป็นเขียวใบไม้
ตามโลโก้ (เดิม `--brass` ไม่มีสีเขียวเลยในชุด token) ปรับ `--paper`/`--surface-sunk`/`--line`/`--ink*` เป็น
โทนครีมอุ่นด้วยเพื่อให้เข้าธีมเดียวกันทั้งหน้า — **คงชื่อตัวแปร CSS เดิมทั้งหมด** ตามแบบแผนที่ ADR-047
วางไว้ (แค่เปลี่ยนค่า ไม่เปลี่ยนชื่อ) เพื่อไม่ต้องไล่แก้ Tailwind class `bg-celadon-*`/`bg-brass-*` ทั่วระบบ
เพิ่มโลโก้ (`apps/web/public/logo.webp`) เข้าไปที่หน้า login และ header ของ `authenticated-shell.tsx`

เหตุผล: การใช้ token slot เดิม (สลับค่าแทนสร้างใหม่) ทำให้ทุก component ที่มีอยู่แล้วได้สีใหม่ทันทีโดยไม่ต้อง
แก้โค้ด component แม้แต่จุดเดียว — สอดคล้องกับกฎ "ห้ามสร้างสีใหม่นอกไฟล์ tokens.css" ของ CLAUDE.md ค่า
contrast (AA 4.5:1) ถูกตรวจสอบให้ยังผ่านเช่นเดิม (ทอง `#8B6227` บนพื้นครีม `#F8F5EE`/ขาว และเขียว
`#5F7A52` บนพื้นเดียวกัน อยู่ในระดับความเข้มใกล้เคียงค่าเดิมที่ผ่าน AA อยู่แล้ว)

ผลกระทบ/ทางเลือกที่ไม่เลือก: **ยังไม่ได้ตรวจ contrast จริงด้วยเครื่องมือวัด** (แค่ประมาณจากความเข้มสี) ควร
รันเครื่องมือเช็ค AA จริงก่อนขึ้น production เต็มรูป — สถานะ "กำลังบริการ" (`in_service` badge) ที่เคยใช้
`--celadon-solid` (ทีลเข้ม) จะกลายเป็นทองเข้มแทน ถ้าดูแล้วสับสนกับสีอื่นในกระดานคิว (Lane Board) ต้อง
revisit เลือก semantic color อื่นแทน — ไม่ได้ทำ wording simplification ทั้งระบบในรอบนี้ (สโคปใหญ่เกินไป
สำหรับ 1 task) ทำแค่การเพิ่มโลโก้ + เปลี่ยนสี เป็น task แยกที่ควรทำเป็นรอบถัดไป

---

## ADR-058: เพิ่ม "ย้าย/เลื่อนเวลา" แบบกดในชีทรายละเอียด — ทางเลือกแทนลากบนมือถือ
วันที่: 2026-09-20
Task ที่เกี่ยวข้อง: T4.5 (Lane Board) ส่วนต่อขยายสำหรับมือถือ

บริบท: `LaneBoard` (จอ ≥ 768px, `md:flex`) ใช้ลาก-วาง (`draggable`) เพื่อย้ายนัดไปพนักงาน/ห้องอื่นและ
เลื่อนเวลา แต่ `MobileAgenda` (จอ < 768px, `md:hidden`) เป็น tap-based ล้วน ไม่มี drag เลย และไม่เคยมีทาง
ทำ 2 อย่างนี้บนมือถือมาก่อนเลย (`AppointmentDetailSheet` เดิมมีแค่ปุ่มเปลี่ยนสถานะ) — ละเมิด WCAG 2.2
`dragging-alternative` (ทุก action ที่ใช้ drag ต้องมีทางเลือกแบบ single-pointer/keyboard ด้วย)

ตัดสินใจ: เพิ่มส่วน "ย้าย/เลื่อนเวลา" ใน `AppointmentDetailSheet` (ใช้ร่วมกันทั้ง mobile/desktop เพราะ
component เดียวกัน ไม่ต้องแยกเขียน 2 ที่) — กด "ย้าย/เลื่อนเวลา" เปิดฟอร์ม เลือกพนักงาน/ห้องใหม่จาก `rows`
เดิมที่ `BoardPageClient` มีอยู่แล้ว (props เดียวกับที่ส่งให้ `LaneBoard`) + `<input type="time">` เลือกเวลา
เริ่มใหม่ (คงระยะเวลาให้บริการเท่าเดิมเสมอ ไม่ให้ผู้ใช้พิมพ์เวลาจบเองเพื่อกันพิมพ์ผิดจนสั้น/ยาวกว่าที่ควร)
เรียก `onReschedule` เดียวกับที่ `handleReschedule` ใช้ตอนลากอยู่แล้ว (ผ่าน `rescheduleMutation` เดิม
ไม่มี endpoint ใหม่) — ส่วน "ยกเลิกคิว" ไม่ต้องทำอะไรเพิ่ม เพราะ `CANCELLED` อยู่ใน `nextAppointmentStatuses`
อยู่แล้วและปุ่มเปลี่ยนสถานะเดิมใช้ได้บนมือถือปกติทุกจุด

เหตุผล: ใช้ backend endpoint/mutation เดิมทั้งหมด (`rescheduleAppointmentItemSchema`,
`PATCH .../reschedule`) ไม่ต้องแตะ `apps/api` เลย ลดความเสี่ยง — คงระยะเวลาบริการเดิมเสมอเพราะ backend
ไม่เช็คว่าระยะเวลาที่เปลี่ยนสมเหตุสมผลกับบริการนั้นไหม (เช่นนวด 60 นาที ผู้ใช้พิมพ์จบเป็น 5 นาทีได้ถ้าเปิดให้
กรอกเอง) ปล่อยให้แก้ระยะเวลาจริงได้เฉพาะทางแก้บริการ/สร้างนัดใหม่แทน

ผลกระทบ/ทางเลือกที่ไม่เลือก: **"จองคิวล่วงหน้า" (สร้างนัดอนาคต ไม่ใช่ walk-in วันนี้) ยังไม่มีอยู่ในระบบเลย
ทั้งมือถือและเดสก์ท็อป** — `WalkInSheet`/`createWalkInAppointmentSchema` รองรับแค่จองด่วนวันนี้ (ระบบเลือก
พนักงาน/ห้อง/เวลาให้อัตโนมัติจากคิวหมุน) ไม่มีฟอร์มเลือกวันที่/เวลา/พนักงาน/ห้องเองสำหรับนัดล่วงหน้าเลย —
เป็น task ใหญ่แยกต่างหาก (ต้อง UI ค้นหาสมาชิก + เช็ค availability ล่วงหน้า) ไม่ได้ทำในรอบนี้

---

## ADR-059: เจ้าของร้านตั้งรหัสผ่านใหม่ให้ user คนอื่นโดยตรง (ไม่ผ่านอีเมล)
วันที่: 2026-09-20
Task ที่เกี่ยวข้อง: "Master user เห็นเฉพาะ admin reset password ได้" (คำสั่งจากผู้ใช้)

บริบท: ระบบยังไม่มี flow "ลืมรหัสผ่าน" ผ่านอีเมลเลย (ไม่มีระบบส่งอีเมล reset ในโปรเจกต์นี้ตอนนี้ — ดู
`.env.example` ที่มีแค่ `SMTP_HOST` สำหรับ mailpit dev เท่านั้น) เมื่อ user ลืมรหัสผ่านจะเข้าระบบไม่ได้เลย
ไม่มีทางแก้ด้วยตัวเอง ผู้ใช้ยืนยันหลังคุยรายละเอียด (ไม่ใช่สร้างบทบาทใหม่ "Master" — ใช้บทบาท owner เดิม)
ว่าต้องการฟีเจอร์ตั้งรหัสผ่านใหม่ตรง ๆ ให้ user คนอื่น เห็นเฉพาะ owner เท่านั้น (ไม่จำกัดว่ารีเซ็ตได้แค่บาง
บทบาท — owner รีเซ็ตให้ user คนไหนก็ได้ในสาขาที่ตัวเองสังกัด) เมนูเดิมของ owner ไม่เปลี่ยน แค่เพิ่มเมนูใหม่

ตัดสินใจ: เพิ่ม `PATCH /branches/:branchId/users/:userId/password` (`branch.controller.ts`) เกทด้วย
`settings:manage` ที่มีแค่ owner ตาม `MANAGE_ALL_EXCEPT_SETTINGS` ใน `packages/contracts/src/permissions.ts`
(ไม่ต้องเพิ่ม permission key ใหม่ ใช้ตัวที่ออกแบบไว้สำหรับ "owner เท่านั้น" อยู่แล้ว) เช็คว่า user เป้าหมาย
สังกัดสาขานี้จริงก่อนเสมอ (404 ถ้าไม่ใช่ — กัน owner สาขา A ตั้งรหัสผ่านให้ user สาขา B ผ่าน branchId ที่ตัวเอง
ไม่ได้สังกัด แม้ PermissionGuard จะกันชั้นแรกไปแล้วก็ตาม) รีเซ็ตสำเร็จแล้วเรียก `AuthService.logoutAll()`
เพิกถอน refresh token เดิมทั้งหมดของ user คนนั้นทันที เพิ่มเมนู "รีเซ็ตรหัสผ่านผู้ใช้" ใน `NAV_ITEMS` กลุ่ม
"จัดการร้าน" require `manage`+`settings` เหมือนกัน — role อื่นไม่เห็นเมนูนี้เลย (เมนูเดิมของ owner ไม่เปลี่ยน)

เหตุผล: reuse permission `settings:manage` แทนสร้าง resource ใหม่ เพราะความหมายตรงกันเป๊ะอยู่แล้ว (ตั้งใจ
ออกแบบให้เป็น "owner เท่านั้น" มาตั้งแต่ ROLE_PERMISSIONS) เพิกถอน session เดิมหลังรีเซ็ตเพื่อความปลอดภัย —
ถ้ารหัสผ่านเดิมหลุด/ถูกขโมย session ที่ค้างอยู่ต้องใช้ต่อไม่ได้หลัง owner รีเซ็ตให้

ผลกระทบ/ทางเลือกที่ไม่เลือก: ปฏิเสธการสร้างบทบาท "master" ใหม่ตามชื่อที่ผู้ใช้ใช้ตอนแรก เพราะคุยรายละเอียด
แล้วพบว่าจริง ๆ ต้องการแค่จำกัดสิทธิ์ให้ owner เท่านั้น ซึ่ง RBAC เดิมรองรับอยู่แล้วผ่าน `settings:manage` ไม่
จำเป็นต้องเพิ่มความซับซ้อนของระบบ role — ยังไม่มีการแจ้งเตือนรหัสผ่านใหม่ให้ user อัตโนมัติ (ไม่มีอีเมล/SMS
ในระบบ) owner ต้องแจ้งรหัสผ่านใหม่ให้เจ้าตัวเองผ่านช่องทางที่ปลอดภัย (หน้าเว็บเตือนไว้เป็นข้อความ)

---

## ADR-060: เพิ่ม/แก้ไข/ปิดใช้งาน user เต็มรูป — ต่อยอดจากหน้ารีเซ็ตรหัสผ่าน (ADR-059)
วันที่: 2026-09-20
Task ที่เกี่ยวข้อง: "เพิ่ม add edit delete user" (คำสั่งจากผู้ใช้ ต่อยอดจาก ADR-059)

บริบท: หน้า "รีเซ็ตรหัสผ่านผู้ใช้" (ADR-059) มีแค่ list + reset password ยังสร้าง/แก้ไข/ปิดใช้งาน user ไม่ได้
เลย (comment เดิมใน `branch.controller.ts` บอกไว้ตรง ๆ ว่า "ยังไม่มี CRUD ผู้ใช้ในระบบตอนนี้") ผู้ใช้ขอเพิ่ม
ให้ครบ

ตัดสินใจ: เพิ่ม `POST /branches/:branchId/users` (สร้าง user ใหม่ + ผูกกับสาขานี้ทันที) และ
`PATCH /branches/:branchId/users/:userId` (แก้ชื่อ/อีเมล/บทบาท/isActive) ทั้งคู่เกทด้วย `settings:manage`
เหมือน endpoint reset password เดิม — "ลบ" คือส่ง `isActive: false` (soft delete ตาม pattern เดิมของ
staff/room/service ทั้งระบบ) ปิดใช้งานแล้วเรียก `AuthService.logoutAll()` เพิกถอน session เดิมเหมือนตอน
รีเซ็ตรหัสผ่าน `roleKey` ใช้ `z.enum` จาก `ROLE_DEFINITIONS` (4 ค่าคงที่ที่ seed ไว้แล้ว — owner/manager/
cashier/staff) ไม่ใช่ roleId ตรง ๆ กันส่ง id ที่ไม่มีจริงมาได้ตั้งแต่ validation `GET /users` เพิ่ม query
`?isActive=all` (ไม่ใส่ = พฤติกรรมเดิม active-only ที่ T5.6 ใช้เลือก "ผู้จัดการที่จะอนุมัติ" อยู่แล้ว ห้ามพัง)
ให้หน้าจัดการเห็น user ที่ปิดใช้งานด้วยเพื่อเปิดกลับได้ ไม่มีฟิลด์รหัสผ่านในฟอร์มแก้ไข — ตั้งรหัสผ่านใหม่แยก
ไปที่ endpoint/ปุ่มของ ADR-059 เดิมเท่านั้น กันแก้ข้อมูลทั่วไปพลาดรีเซ็ตรหัสผ่านไปด้วยโดยไม่ตั้งใจ

เหตุผล: reuse `settings:manage` เดิมแทนสร้าง permission ใหม่ (สม่ำเสมอกับ ADR-059) soft delete แทน hard
delete เพราะ user ผูกกับ audit log/บิลที่ยกเลิก/รอบกะที่เปิด-ปิด ฯลฯ อยู่มาก (ดู relations ใน schema.prisma)
ลบแถวจริงจะทำให้ประวัติเก่าอ้างอิง user ที่หายไปพัง

ผลกระทบ/ทางเลือกที่ไม่เลือก: ยังไม่มีการยืนยันอีเมลตอนสร้าง user ใหม่ (owner กรอกอีเมล/รหัสผ่านเริ่มต้นให้เอง
...
## ADR-063: เปิด CRUD หมวดบริการโดยไม่เปิดการจัดลำดับเอง

วันที่: 2026-09-21
Task ที่เกี่ยวข้อง: เพิ่มหน้าจัดการหมวดบริการตามรูปแบบประเภทห้อง

บริบท: เดิม ADR-012 ให้ ServiceCategory เป็นรายการอ่านอย่างเดียวจาก seed แต่ร้านต้องเพิ่มและแก้ชื่อหมวดบริการเองได้ เช่น นวด สปา หรือเล็บ รูปแบบการใช้งานเหมือนการจัดการ RoomType ตาม ADR-062

การตัดสินใจ: เปิด POST/PATCH/DELETE ที่ `/branches/:branchId/service-categories` ด้วยสิทธิ์ `service:manage` และเพิ่ม sheet “จัดการหมวดบริการ” ในหน้าบริการ ชื่อซ้ำภายในสาขาคืน 409; การแก้/ลบข้ามสาขาคืน 404; ลบจริงได้เฉพาะหมวดที่ไม่มี Service อ้างอิง โดยเช็คก่อนและจับ foreign-key race เป็น 409 ทุก mutation ผ่าน AuditInterceptor

หมวดใหม่ใช้ `max(sortOrder) + 1` เพื่อไปต่อท้ายรายการเดิม ส่วนการ reorder ยังไม่อยู่ในขอบเขตนี้ จึงไม่ต้องแก้ schema หรือสร้าง migration

---

## ADR-064: อนุญาตให้ยกเลิกนัดที่เช็คอินแล้ว (CHECKED_IN → CANCELLED) — แก้ช่องโหว่ของ ADR-021
วันที่: 2026-09-21
Task ที่เกี่ยวข้อง: "จองด่วนควรยกเลิกได้ด้วย" (รายงานจากผู้ใช้จริงหลังทดสอบจองด่วน)

บริบท: ADR-021 ล็อกกติกาว่า "ยกเลิก/ไม่มา ทำได้เฉพาะก่อนเช็คอินเท่านั้น (BOOKED/CONFIRMED) — เช็คอินแล้วถือว่า
ลูกค้ามาจริงแล้ว" ตอนออกแบบตอนนั้นคิดถึงเส้นทางปกติ BOOKED→CONFIRMED→CHECKED_IN แต่ "จองด่วน" (T4.6,
`createWalkIn`) สร้างนัดเริ่มต้นที่สถานะ **CHECKED_IN ทันที** (ลูกค้ายืนอยู่หน้าร้านแล้ว ไม่ต้องผ่านขั้นจอง/
ยืนยันเหมือนนัดล่วงหน้า) ผลคือจองด่วนทุกใบไม่มีทางยกเลิกได้เลยแม้แต่ทางเดียว — ถ้าพนักงานกดผิดบริการ/ผิดคน
หรือลูกค้าเปลี่ยนใจก่อนเริ่มนวดจริง ระบบไม่มีทางแก้ไขให้เลย ต้องปล่อยค้างเป็น "เช็คอินแล้ว" ตลอดไป ผู้ใช้ทดสอบ
จองด่วนแล้วเจอปัญหานี้จริง

ตัดสินใจ: เพิ่ม `CANCELLED` เข้า transition ที่อนุญาตจาก `CHECKED_IN` ใน `APPOINTMENT_TRANSITIONS`
(`packages/contracts/src/booking.ts`) — **ไม่เพิ่ม** `NO_SHOW` เข้าไปด้วย เพราะเช็คอินแล้วแปลว่าลูกค้ามาจริง
แน่นอน (ไม่มีทาง "ไม่มา" ได้อีกต่อไปหลังเช็คอิน) `IN_SERVICE` ยังคงห้ามยกเลิกเหมือนเดิม (เริ่มนวดจริงแล้วย้อน
ไม่ได้) ไม่ต้องแก้โค้ด backend/frontend อื่นเลยนอกจาก pure function นี้ตัวเดียว — `updateStatus` endpoint ใช้
`canTransitionAppointmentStatus` ตรง ๆ อยู่แล้ว, `applyQueueEffect` รับสถานะ CANCELLED แบบ generic ไม่สนใจ
สถานะเดิมอยู่แล้ว, และปุ่มเปลี่ยนสถานะใน `AppointmentDetailSheet` render จาก `nextAppointmentStatuses()`
โดยตรงอยู่แล้วเช่นกัน — เปลี่ยนที่ pure function จุดเดียวไหลไปทั้งระบบอัตโนมัติ

เหตุผล: การแก้ที่ layer เดียว (contracts) แทนแก้หลายจุดกัน logic เพี้ยนไม่ตรงกันระหว่าง frontend/backend —
ตรงกับเจตนาเดิมของ `canTransitionAppointmentStatus` ที่ออกแบบให้เป็น single source of truth ของกติกาข้าม
สถานะอยู่แล้ว

ผลกระทบ/ทางเลือกที่ไม่เลือก: อัปเดต test เดิมใน `appointment-item-status.e2e-spec.ts` ที่ assert พฤติกรรมเก่า
(CHECKED_IN→CANCELLED ต้อง 422) ให้ตรงกับพฤติกรรมใหม่ (200) และแยกเทสต์ใหม่ยืนยันว่า IN_SERVICE→CANCELLED
ยังคงต้อง 422 เหมือนเดิม — รัน e2e ทั้งชุด 257/257 ผ่าน ไม่มี regression อื่น ปฏิเสธการแยกกติกาเฉพาะจองด่วน
(เช่น flag แยกว่า "มาจาก walk-in") เพราะซับซ้อนเกินความจำเป็น — กติกาเดียวกัน (CHECKED_IN ยกเลิกได้ก่อนเริ่ม
บริการ) ใช้ได้สมเหตุสมผลกับทุกเส้นทางที่มาถึง CHECKED_IN ไม่ใช่แค่ walk-in

---

## ADR-065: login ด้วยชื่อผู้ใช้แทนอีเมลได้ — บังคับ User.name เป็นภาษาอังกฤษล้วนและ unique
วันที่: 2026-09-21
Task ที่เกี่ยวข้อง: "ปรับให้ login ด้วยชื่อหรือเมลล์ก็ได้ และตอนเพิ่มชื่อต้องเป็นภาษาอังกฤษเท่านั้น" (คำสั่งจากผู้ใช้)

บริบท: เดิม `loginSchema` บังคับ field "email" ต้องเป็นรูปแบบอีเมลเท่านั้น (`z.string().email()`)
`User.name` ก็ไม่เคย unique มาก่อน (ใช้เป็นแค่ชื่อแสดงผล ซ้ำกันได้) ผู้ใช้ขอให้ login ด้วยชื่อผู้ใช้แทนอีเมลได้
ด้วย ซึ่งทำให้ชื่อต้องกลายเป็น identifier ที่ไม่ซ้ำกันจริง — และขอให้บังคับภาษาอังกฤษล้วนตอนกรอกชื่อ (ป้องกัน
ปัญหา keyboard layout ภาษาไทยตอน login ที่อุปกรณ์บางตัว/ลดความกำกวมกับชื่อพนักงาน `StaffProfile.name` ที่
เป็นคนละ entity และยังเป็นภาษาไทยได้ตามปกติ)

ตัดสินใจ:
- `packages/db/prisma/schema.prisma`: เพิ่ม `@unique` ให้ `User.name` (migration
  `20260921033702_user_name_unique` — สร้างด้วยมือเพราะ `prisma migrate dev` ต้องการ interactive
  terminal ที่ sandbox นี้ไม่รองรับ ทดสอบแล้วว่า apply ผ่านกับ dev DB จริงไม่มี error)
- `loginSchema` (`packages/contracts/src/auth.ts`): เปลี่ยนจาก `.email()` เป็น `.min(1)` เฉย ๆ — **คง
  ชื่อ field เป็น "email" เหมือนเดิม** ไม่เปลี่ยนเป็น "identifier" เพราะจะกระทบ e2e spec ~30 ไฟล์ที่ยิง
  `POST /auth/login` ตรง ๆ โดยไม่จำเป็น ให้ backend (`AuthService.validateCredentials`) ลองหาด้วย email
  ก่อนเสมอ ไม่เจอค่อยลองด้วย name แทน
- `createUserSchema`/`updateUserSchema` (`packages/contracts/src/users.ts`): เพิ่ม regex บังคับชื่อขึ้นต้น
  ด้วยตัวอักษร A-Z ตามด้วย A-Z/0-9/เว้นวรรค/จุด/ขีดกลาง/อะพอสทรอฟีเท่านั้น — บังคับที่ application layer
  (ไม่ใช่ DB) เพราะ `StaffProfile.name` (คนละ entity ใช้ภาษาไทยปกติ) ไม่ต้องถูกจำกัดด้วย
- `branch.controller.ts`: แยกข้อความ error ตอนชนกันของ `createUser`/`updateUser` ด้วย `err.meta.target`
  จาก Prisma P2002 — บอกว่าอีเมลซ้ำหรือชื่อซ้ำให้ตรงจริง ไม่เดาแบบเดิมที่ขึ้น "อีเมลนี้มีผู้ใช้อยู่แล้ว" เสมอ

เหตุผล: คง field "email" เดิมไว้ (ไม่ rename) เพื่อลด blast radius ของการเปลี่ยนแปลงให้เหลือน้อยที่สุด —
ระบบเดิมทำงานถูกต้องอยู่แล้วสำหรับ login ด้วยอีเมล การเพิ่มความสามารถใหม่ไม่ควรบังคับแก้โค้ดที่ทำงานถูกอยู่
แล้วหลายสิบจุดโดยไม่จำเป็น — บังคับภาษาอังกฤษที่ระดับ Zod schema (ไม่ใช่ DB constraint) เพราะยืดหยุ่นกว่า
และให้ error message อ่านรู้เรื่องได้ทันทีจากฝั่ง client ก่อนยิง request ด้วยซ้ำ

ผลกระทบ/ทางเลือกที่ไม่เลือก: ต้องรัน `prisma migrate deploy` บน **production Neon DB ด้วยตัวเองเท่านั้น**
(agent รันคำสั่งที่แตะ credential ของ production database ไม่ได้ — ติด permission classifier เหมือนตอน
ADR-059/060) — ก่อน deploy ต้องเช็คว่า production ไม่มี `User.name` ซ้ำกันอยู่แล้ว (ถ้ามี migration จะ fail
ทันทีตอน apply ไม่กระทบข้อมูลเดิม แค่ apply ไม่สำเร็จ) แก้ test เดิมใน `user-crud.e2e-spec.ts` ที่ใช้ชื่อไทย
("พนักงานใหม่" ฯลฯ) เป็นภาษาอังกฤษทั้งหมด เพราะตอนนี้ผ่าน schema เดิมไม่ได้แล้ว — เพิ่ม 3 e2e test ใหม่
(login ด้วยชื่อ, ชื่อซ้ำ 409, ชื่อไทย 400) รวม e2e ทั้งระบบ 260/260 ผ่าน ไม่มี regression — ปฏิเสธการบังคับ
unique/ภาษาอังกฤษกับ `StaffProfile.name` ด้วย เพราะเป็นคนละวัตถุประสงค์ (ชื่อแสดงผลลูกค้าเห็น ไม่ใช่ login
identifier) การบังคับจะทำให้พนักงานที่ใช้ชื่อไทยอยู่แล้วเดือดร้อนโดยไม่จำเป็น
