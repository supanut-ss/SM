...
## ADR-032: ลงเวลาทำงาน (T6.1) — ผูก StaffProfile กับ User ผ่านฟิลด์ใหม่, ใช้ session PIN เดิมแทน one-off PIN, จับคู่กะที่ startMin ใกล้เวลาเข้างานจริงที่สุด, ไม่หักเงินจากสาย/ขาด

วันที่: 2026-08-25
Task ที่เกี่ยวข้อง: T6.1 (M6 เริ่มต้น — T6.2/T6.3/T6.4 ยังไม่เริ่มเพราะรอตัวเลขค่ามือจริงจากเจ้าของร้าน
ตามที่ docs/DOMAIN.md บล็อกไว้ ดูหัวข้อ "สถานะ" ในไฟล์นั้น)

บริบท: PLAN.md ระบุแค่ "clock in/out ด้วย PIN ที่เครื่องหน้าร้าน, คำนวณสาย/ขาด/OT เทียบกะ" เกณฑ์ผ่านมีข้อ
เดียวคือ "ลงเวลาซ้ำในนาทีเดียวกันต้องถูกปฏิเสธ" — กลไกจริงเบื้องหลัง (จะยืนยันตัวตนอย่างไร, จับคู่กะอย่างไร,
"ขาด" นับยังไง) ไม่มีในเอกสารเลย ต้องตัดสินใจเองทั้งหมดในเซสชันนี้

ตัดสินใจ:
1. **เพิ่ม `StaffProfile.userId` (nullable, unique, FK ไป `User`) แทนการสร้างกลไก PIN แยกใหม่** — พบว่า
   `packages/contracts/src/permissions.ts` seed สิทธิ์ `attendance:manage` ให้บทบาท "พนักงานบริการ" ไว้ตั้งแต่
   T1.1 อยู่แล้ว (ไม่มีใครใช้จนถึงตอนนี้) แปลว่าการออกแบบเดิมตั้งใจให้พนักงานล็อกอินแบบ PIN ปกติ (T1.3, session
   8 ชม.) แล้วใช้สิทธิ์บทบาทตัวเองยิง endpoint นี้ตรง ๆ ไม่ใช่กลไก one-off PIN แบบ `verifyManagerPin` (T5.6) —
   แต่ `StaffProfile` (ตารางที่ผูกกับกะ/ค่ามือ) กับ `User` (ตารางที่มี pinHash) ไม่เคยมีความสัมพันธ์กันมาก่อน
   เพิ่ม FK นี้เพื่อให้ resolve "userId ที่ authenticate แล้ว → StaffProfile ไหน" ได้ ผูก/แก้ได้ผ่าน
   `StaffController` (create/update) เหมือนฟิลด์อื่น — ไม่ได้ทำ endpoint ผูกบัญชีแยกต่างหาก
2. **"me"/"clock-in"/"clock-out" ใช้สิทธิ์ `manage` ไม่ใช่ `view`, ส่วน list/summary ใช้ `view`** — ตรงกับสิทธิ์
   ที่บทบาท "พนักงานบริการ" มีอยู่แล้ว (`attendance:manage` เท่านั้น) พบระหว่างเขียนโค้ดว่า `ability.factory.ts`
   ให้ "manage" ครอบคลุม "view" เสมออยู่แล้วทั้งระบบ ("จัดการได้ย่อมดูได้") พนักงานเลยเห็นรายงานสรุป (list/
   summary) ได้ด้วยโดยไม่ตั้งใจ — ยอมรับพฤติกรรมนี้แทนที่จะฝืนสร้างกลไกกันแยกเพิ่ม เพราะร้านเล็กสาขาเดียว
   พนักงานเห็นตารางกะ/สถานะวันนี้ของเพื่อนร่วมงานไม่ใช่ความเสี่ยงจริง และขัดกับ convention ของทั้งระบบถ้าฝืนกัน
3. **จับคู่เวลาที่ลงเข้างานกับ `StaffShift` ที่ `startMin` ใกล้เคียงที่สุด (ทั้งมาก่อน/มาหลัง) ในบรรดากะที่ยัง
   ไม่ถูกจับคู่ในวันนั้น** (`packages/core/attendance/matchShiftForClockIn`) — รองรับกะแบ่งครึ่ง (คร่อมพักเที่ยง
   ตามที่ StaffShift schema comment ของ T2.4 อนุญาตไว้) ไม่มีกะเหลือให้จับคู่ = ลงเวลานอกตาราง (`staffShiftId`
   null, ไม่คำนวณสาย/OT เพราะไม่มีเส้นฐาน) กะที่ตรงกันถูกจับคู่แล้วจะไม่ถูกเลือกซ้ำโดยพนักงานคนเดียวกันในวันนั้น
4. **"ขาด" ไม่ใช่ค่าที่เก็บบนแถว `AttendanceRecord` แต่เป็นค่าที่คำนวณสด (`summarizeDailyShiftStatus`) จากการ
   เทียบ `StaffShift` ของวันนั้นกับ `AttendanceRecord` ที่มีอยู่** — เพราะ "ขาด" คือการไม่มีแถวเลย ไม่มีอะไรให้
   UPDATE เก็บสถานะนี้ตรง ๆ ผู้เรียกกำหนด `nowMinutesOfDay` เอง (นาทีปัจจุบันถ้าสรุปวันนี้, 1440 ถ้าสรุปวันที่
   ผ่านไปแล้วให้ถือว่าขาดเสมอ) กันฟังก์ชันต้องเรียก `Date.now()` เอง (กฎเหล็ก CLAUDE.md ข้อ 1)
5. **ลงเวลาเข้างานใหม่ระหว่างที่มีรอบเปิดอยู่แล้วถูกบล็อกเสมอ (ต้องออกงานก่อน)** — ไม่ได้ระบุไว้ใน PLAN.md
   แต่จำเป็นเพื่อให้ "รอบล่าสุด" ของพนักงานมีลำดับเวลาแน่นอนเสมอ (ใช้เป็นฐานของเช็ค "ลงเวลาซ้ำในนาทีเดียวกัน"
   ข้อ 6 ด้วย) — ป้องกัน state ที่มีหลายรอบเปิดพร้อมกันของพนักงานคนเดียว ซึ่งจะทำให้คำนวณสาย/OT/ขาด สับสน
6. **เกณฑ์ผ่าน "ลงเวลาซ้ำในนาทีเดียวกันต้องถูกปฏิเสธ" เช็คจากเหตุการณ์ล่าสุดของพนักงานคนนั้นเท่านั้น** (เข้า
   ล่าสุดถ้ารอบยังเปิดอยู่ ออกล่าสุดถ้ารอบปิดไปแล้ว) ผ่าน `isSameMinute` (`packages/core/attendance`, timezone-
   invariant) — ครอบทั้งกดเข้างานซ้ำและกดออกงานแล้วกดเข้าใหม่ทันทีในนาทีเดียวกัน ไม่ใช่แค่ปุ่มเดียวกัน
7. **ไม่มีค่าปรับหักเงินจากสาย/ขาด/OT เด็ดขาด (`lateMinutes`/`otMinutes`/`earlyLeaveMinutes` เก็บไว้เพื่อ
   รายงาน/ตักเตือนเท่านั้น)** — ตรงกับ docs/DOMAIN.md ข้อ 13 ตรง ๆ ("ไม่มีค่าปรับ — ใช้ระบบตักเตือนแทน")

เหตุผล: ข้อ 1–2 ใช้ประโยชน์จากการออกแบบสิทธิ์ที่ seed ไว้ล่วงหน้าตั้งแต่ T1.1 (attendance:manage ของบทบาท
"พนักงานบริการ") แทนที่จะสร้างกลไกใหม่ขนาน ลดพื้นผิวโค้ด/ความเสี่ยงด้าน auth ข้อ 3–6 ยึดหลักเดียวกับ
packages/core/availability (T4.1): เวลาทั้งหมดคำนวณเป็น pure function รับ `now`/เวลาเป็น parameter เสมอ
ไม่เรียก `Date.now()` ในนั้น ห้าม import Prisma — unit test ควบคุมเวลาได้แน่นอน 100% (ต่างจาก e2e ที่ควบคุม
เวลาจริงไม่ได้ จึงแบ่งงานทดสอบ: ตรรกะเวลาละเอียด → unit test ใน packages/core, การเชื่อมต่อ/สิทธิ์/state
machine → e2e)

ผลกระทบ/ทางเลือกที่ไม่เลือก: ทางเลือกที่ไม่เลือกคือ (1) สร้าง one-off PIN endpoint แยกแบบ `verifyManagerPin`
สำหรับลงเวลาโดยเฉพาะ — ปฏิเสธเพราะสิทธิ์ `attendance:manage` ที่ seed ไว้แล้วบอกชัดว่าตั้งใจให้ใช้ session PIN
ปกติอยู่แล้ว สร้างซ้ำจะเป็น "endpoint ที่เอี่ยงจากที่ seed ไว้" (2) ผูก `User.staffProfileId` แทน (ฝั่ง User) —
ปฏิเสธเพราะ `StaffController.update()` เป็น endpoint เดียวที่มีอยู่แล้วสำหรับแก้ StaffProfile และรับ field ตรง
ผ่าน `data: body` ได้เลย ผูกฝั่ง `StaffProfile.userId` ทำให้ไม่ต้องเขียน service แยกสำหรับแก้ฝั่ง `User` (3)
เก็บ "ขาด" เป็นคอลัมน์ที่ job รายวันมา backfill — ปฏิเสธเพราะ T7.1 (job สรุปรายวัน) ยังไม่เริ่ม และ "ขาด" คำนวณสด
ได้ตรงไปตรงมาอยู่แล้วไม่จำเป็นต้อง denormalize ตอนนี้ — **ข้อควรระวังสำหรับ Task ในอนาคต**: ฟอร์มผูกบัญชีใน
`StaffForm` (หน้า "พนักงาน") ตอนนี้ "ผูกใหม่" ได้อย่างเดียว ยังกด "เลิกผูก" ไม่ได้ (schema ฝั่ง update รองรับ
`userId: null` แล้วแต่ยังไม่มีปุ่มในฟอร์ม) ถ้าต้องใช้จริงให้เพิ่มตัวเลือก "-- เลิกผูกบัญชี --" ที่ส่ง `null`
แยกจาก "-- ไม่ผูกบัญชี --" (ที่ส่ง `undefined` = ไม่แก้ค่าเดิม) ตอนทำ Task ที่เกี่ยวข้องในอนาคต
